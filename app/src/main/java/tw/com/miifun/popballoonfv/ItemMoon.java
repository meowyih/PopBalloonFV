 ...
 */
public class ItemMoon extends ItemMoving {

    final static private String appTag = "ItemMoon";
    ...
    public ItemMoon(Context context, int type, int x, int y, int width, int height, int screenWidth, int screenHeight ) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                if ( isHit( (int)event.getX(), (int)event.getY(), mWidth / 2, mHeight / 2 ) && ! mIsRotating ) {
                    ...
                    ThreadGaming.playAudioSfx( ThreadGaming.AUDIO_DOWNER );
                }
                break;
